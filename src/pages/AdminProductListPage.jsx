...
const AdminProductListPage = () => {
    ...
    // 1. Hook para cargar los productos reales de la API
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                // Llama a tu API de Express
                const response = await axios.get('http://localhost:5000/api/products'); 
                setProducts(response.data);
            } catch (err) {
                setError('Error de conexión: Asegúrate de que el servidor Express esté corriendo y la configuración CORS sea correcta.');
    ...
    if (loading) {
        return <div className="admin-page-container"><p>Cargando productos de la Base de Datos...</p></div>;
    }

    if (error) {
        return <div className="admin-page-container"><p className="error-message">Error: {error}</p></div>;
    }
                    ...
                    {products.length > 0 ? (
                        products.map((product) => (
                            <tr key={product._id} style={trStyle}>
                                {/* ✅ USO DE CAMPOS CORRECTOS DE MONGODB */}
